...
const DevStudio: React.FC<DevStudioProps> = ({ initialState, onNavigate }) => {
  ...
  if (!initialState) {
    return (
      <Section className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        <div className="p-6 bg-slate-900/50 rounded-full border border-indigo-500/20 shadow-neon-violet mb-6">
        ...
        <div className="space-y-2 mb-8">
            ...
            <p className="font-mono text-sm max-w-md mx-auto text-slate-400">No repository data is currently loaded into the development environment.</p>
        </div>
        <button 
            onClick={() => onNavigate(ViewMode.REPO_ANALYZER)}
            className="px-6 py-3 bg-indigo-500 text-white rounded-xl font-bold hover:bg-indigo-400 transition-all flex items-center gap-2 shadow-lg btn-interaction"
        >
  ...
  return (
    <Section>
        {/* Main Grid-12 Layout */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto lg:h-[calc(100vh-200px)] min-h-[600px]">
        
        {/* Left Pane: Graph (8 cols) */}
        <div className="lg:col-span-8 flex flex-col glass-panel rounded-3xl overflow-hidden h-[500px] lg:h-auto">
        ...
        {/* Right Pane: Terminal (4 cols) */}
        <div className="lg:col-span-4 glass-panel rounded-3xl flex flex-col overflow-hidden h-[500px] lg:h-auto">
                        ...
                        <div className="grid grid-cols-3 gap-2 pt-1">
                            {QUICK_ACTIONS.map((action, idx) => (
                                <button
                                    ...
                                    className="flex flex-col items-center justify-center gap-1 p-2 rounded-lg bg-white/5 hover:bg-indigo-500/20 border border-white/5 hover:border-indigo-500/30 transition-all group disabled:opacity-50 btn-interaction"
                                >
            ...
            {/* Chat History */}
            <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-sm bg-slate-950/30 relative min-h-0 custom-scrollbar">
            ...
            <div className="p-3 bg-slate-950/80 border-t border-white/5 shrink-0">
                <form onSubmit={handleAskQuestion} className="relative flex items-center glass-panel rounded-xl p-1.5 focus-within:ring-1 ring-indigo-500/50 transition-all bg-black/20">
                    ...
                    />
                    <button type="submit" disabled={!questionInput.trim() || chatLoading} className="p-1.5 bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 rounded-lg transition-colors disabled:opacity-0 flex-shrink-0 btn-interaction">
